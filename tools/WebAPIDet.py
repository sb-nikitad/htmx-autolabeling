...
from tools.inference_od import batch_golfer_detect
...
class Inference(object):
    ...
    def __init__(self):
        ...
        self.model_2d_path = 'models/tensorflow_model/lpn_pose2d_tf_38_148k.h5'
        self.model_3d_path = "models/tensorflow_model/attn_videopose3d_33_1000K_deg0_90.h5"
        
        device = torch.device('cuda:0')
        self.model_od = torch.load('models/nanodet.model').to(device)
        self.model_od.eval()
        self.pipeline = Pipeline(pickle.load(open('models/cfg.pipeline', 'rb')), True)
        ...
        with tf.device('/GPU:1'):
            self.model2d = get_pose_net()
            self.model2d(np.zeros((1, 256, 192, 3)))
            self.model2d.load_weights(self.model_2d_path)
        
        with tf.device('/GPU:2'):
            self.model3d = TemporalModelTF(num_joints_in=33, in_features=2, num_joints_out=33,
                                        filter_widths=[3 for i in range(3)])
            self.model3d(np.zeros((1, 27, 33, 3)))
            self.model3d.load_weights(self.model_3d_path)
    ...
    def inference_2d_skeleton(self, image, bbox, flag=0):
        ...
        output_data = self.model2d(input_data)
    ...
    def inference_2d_skeleton_batch(self, image, bboxs, batch_size, flag=0):
        # flag = 0 means RGB format
        if flag == 0:
            image = image
        else:
        ...
        # transformImage = result.get()
            center, scale = box2cs(bboxs[i][0], bboxs[i][1], bboxs[i][2], bboxs[i][3], aspect_ratio)
        ...
        tic = time.time()
        output_data = np.transpose(output_data, (0, 3, 1, 2))
        preds, maxvals = get_final_preds(output_data, centers, scales)
    ...
    def adjust_bbox(self, x, y, w, h, alpha, beta, video_info):
        ...
        w = (1 + 2 * alpha) * w if x + (1 + 2 * alpha) * w < video_info['width'] else video_info['width'] - x
        h = (1 + 2 * beta) * h if y + (1 + 2 * beta) * h < video_info['height'] else video_info['height'] - y
    ...
    def inference_3d_skeleton(self, video_path, ignore_seconds,
        ...
        frames = np.array(frames)
        video_info = {"width": frames.shape[2], "height": frames.shape[1], "frames_rate": frames_rate, "ignore_seconds": ignore_seconds,
                    "frames_num": frames_num}
        
        # 2D inference
        # Fix bbox as the location in the first frame
        # v = skvideo.io.vreader(video_path)
        print("----------Doing 2D Detection----------")

        tic = time.time()
        self.stage = "Detect Golfer"
        self.num_frame = frames_num
        bbox_2d = np.zeros((frames_num, 4))
        batch_size = 16
        for i in range(frames_num // batch_size):
            self.current_frame = i * batch_size
            bbox_2d[i * batch_size: (i + 1) * batch_size] = batch_golfer_detect(frames[i * batch_size: (i + 1) * batch_size], self.model_od, self.pipeline)
        if frames_num % batch_size != 0:
            bbox_2d[(frames_num // batch_size) * batch_size:] = batch_golfer_detect(frames[(frames_num // batch_size) * batch_size:], self.model_od, self.pipeline)
        print()
        print('detect golfer:', time.time() - tic)
        
        if np.sum(bbox_2d[0]) < 1e-4:
            return None, None, None, None
        
        zeros_index = np.where(bbox_2d[:, 2] < 1e-4)[0]
        if len(zeros_index) > bbox_2d.shape[0] * 0.5:
            return None, None, None, None
        
        while(len(zeros_index) > 0):
            bbox_2d[zeros_index] = bbox_2d[zeros_index - 1]
            zeros_index = np.where(bbox_2d[:, 2] < 1e-4)[0]
        ...
        for i, frame in enumerate(frames):
            ...
            if i % batch_size != batch_size - 1 and i + 1 != len(frames):
                ...
                # exit()
                preds_list, conf_list = self.inference_2d_skeleton_batch(input_data[:current_batch], bbox_2d[i - current_batch + 1:i + 1], current_batch)
        ...
        x = np.min(results_2d[0, results_2d[0, :, 0] > 0, 0])
        y = np.min(results_2d[0, results_2d[0, :, 1] > 0, 1])
        ...
        input_data[0, :, :, :] = np.concatenate((normalize_results_2d[:, 1:2, :], normalize_results_2d[:, 5:37, :]), axis=-2)
        skeletons3d = np.zeros((1, frames_num, num_kpts, 3))
        
        output_data = self.model3d(input_data)
        ...
        for i in range(frames_num):
            ...
            try:
                ...
                # print(bbox_2d)
                # print(skeletons3d[:, i:i+1, :, :])
                # print(results_2d[i, :, :])
                # print(i)
                print(bbox_2d[i - 10:i+10])
                exit()
...
if __name__ == "__main__":
    ...
    model3d = TemporalModelTF(num_joints_in=33, in_features=2, num_joints_out=33,
                                    filter_widths=[3 for i in range(3)])
    print(model3d(np.zeros((1, 30, 33, 3))).shape)
