...
class Inference(object):
    ...
    def __init__(self, GPU=[0, 1], num_per_GPU=1, faceOn=False):
        self.statuses = [{'stage': 'IDLE', 'num_frame': 0, 'current_frame': 0, 'req_id': 0} for i in range(len(GPU)*num_per_GPU)]

        if faceOn:
            self.model_2d_path = 'models/lpn_pose2d_tflite_38_140k_v2.tflite'
            self.model_3d_path = "models/tensorflow_model/attn_videopose3d_33_1000K_deg0.h5"
        else:
            self.model_2d_path = 'models/lpn_pose2d_tflite_38_177k.tflite'
        ...
        self.model2d = []
        self.model3d = []
        self.input_details2d = []
        self.output_details2d = []
        
        for i in range(len(GPU)*num_per_GPU):
            self.model2d.append(tf.lite.Interpreter(model_path=self.model_2d_path))
            self.model2d[i].allocate_tensors()
            self.input_details2d.append(self.model2d[i].get_input_details())
            self.output_details2d.append(self.model2d[i].get_output_details())
            with tf.device('/GPU:%d' % GPU[i//num_per_GPU]):
    ...
    def inference_2d_skeleton(self, image, bbox, flag=0, gpu=0):
        ...
        self.model2d[gpu].set_tensor(self.input_details2d[gpu][0]['index'], input_data)
        self.model2d[gpu].invoke()
        output_data = self.model2d[gpu].get_tensor(self.output_details2d[gpu][0]['index'])
    ...
    def inference_3d_skeleton(self, video_path, ignore_seconds,
        ...
        for i, frame in enumerate(frames):
            ...
            if tmp_bbox[0] != 0 and tmp_bbox[1] != 0 and tmp_bbox[2] != 0 and tmp_bbox[3] != 0:
                x, y, w, h = tmp_bbox
                tmp_bbox = self.adjust_bbox(x, y, w, h, 0.15, 0.1, video_info)
        ...
        if smooth2d:
            ...
            if smooth_type == 'polynomial':
                ...
                buildInfo = {
                        ...
                        "unity_version": "None",
                        "pose2DModel_version": "lpn_pose2d_tflite_38_140k_v2",
                        "pose3DModel_version": "attn_videopose3d_33_1000K_deg0",
                        "app_buildType": "None",
                        "system_info": "None"
                    }
        ...
        if camera_intrinsics is None:
            default_fx, default_fy = 1300, 1300
            cx, cy = video_info["width"] // 2, video_info["height"] // 2
            default_cx, default_cy = 960 if cx > cy else 540, 540 if cx > cy else 960
