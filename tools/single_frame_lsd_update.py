...
def update_lsd(results_2d, conf_2d, img_path):
...
if __name__ == '__main__':
    image_root = './data/golf/images/custom_val'
    json_root = './tools'
    json_path = os.path.join(json_root, 'keypoints_custom_val_results_0.json')
    output_json_path = os.path.join(json_root, 'keypoints_custom_val_results_0_updated.json')

    json_file = json.load(open(json_path))
    updated_json = copy.deepcopy(json_file)
    ...
    for i in range(len(updated_json)):
        ...
        image_path = os.path.join(image_root, '%06d.jpg' % item['image_id'])
        results_2d = np.array(item['keypoints']).reshape((-1, 3))
        conf_score = results_2d[:, 2][:, np.newaxis]
        skeleton_2d = results_2d[:, :2]

        if conf_score[36, 0] < 0.8:
            skeleton_2d, conf_score = update_lsd(skeleton_2d, conf_score, image_path)
        results_2d = np.concatenate((skeleton_2d, conf_score), axis=-1)
        item['keypoints'] = results_2d.reshape(-1).tolist()

    json.dump(updated_json, open(output_json_path, 'w'))
